' Finds all the digits of the n'th number in the fibonacci sequence.
...
' To compile with freebasic, use the command fbc fib_fb.bas

' Function declarations
declare function add (a as string, b as string) as string
declare function multi (a as string, b as string) as string
declare function pow (x as long, n as long) as string
declare function pow2 (x as long, n as long) as string
declare function fib (n as long) as string
declare function fact (n as long) as string
...
' print pow2(2, val(command$(1)))
print fib$(val(command$(1)))
...
function add(a as string, b as string) as string
    ...
    for i = 0 to lena - 1
        ...
        if lenb - i >= 0 then
            ...
            if cm > 0 then
                m = val(mid$(a, lena - i, 1)) + cm
            else
                lf = mid$(a, 1, lena - i)+lf
                exit for
            endif
        endif
        if m >= 10 then
            ...
            am = m mod 10
            ' The tens digit is never greater than 1
            cm = 1
        else
...
function multi(a as string, b as string) as string
    ...
    for i = 0 to lenb - 1
        ...
        if ef <> "" then
            ...
            ec = mid$(ef, (len(ef) - i) + 1, i)
            ef = mid$(ef, 1, (len(ef) - i))
            ' Add the current temporary string to the sum of the last, and
            ' append the dropped digits on the end.
            ef = add(lfb, ef) + ec
        else
...
function pow(x as long, n as long) as string
...
function pow2(x as long, n as long) as string
    ' pow2$(x, n) -> Given the two integers x and n, return a string
    ' containing all the digits of x raised to the nth power.
    ...
    ' Bitmask to check for ones bit in n.
    t = 1
    do while t <= n
        ' Check if the t'th bit is a 1 and multiply z and y if it is.
        if n and t = t then
            z = multi$(z, y)
        endif
        ' Square y.
        y = multi$(y, y)
        ' Shift t 1 bit to the left, so the next bit of n can be checked.
        t = t * 2
    loop
...
function fib(n as long) as string
...
function fact(n as long) as string
